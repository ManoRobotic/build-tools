...
class RzavalaDBFUploader:
    ...
    def map_opro_record_to_api(self, record: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if not product_key:
                logger.warning(f"Record with NO_OPRO {no_opro} has empty CVE_PROD and CVE_PROP")
    ...
    def resync_product_keys(self) -> bool:
        """
        Re-procesa TODAS las órdenes del DBF y actualiza product_key en el WMS
        usando el endpoint PATCH /api/production_orders/batch_update.

        Útil para corregir órdenes ya subidas con códigos internos (PROMAT*).
        Ejecutar con: rzavala_dbf_uploader.exe --resync
        """
        try:
            logger.info("=" * 60)
            logger.info("RESYNC PRODUCT KEYS - Re-procesando todas las órdenes")
            logger.info("=" * 60)
            ...
            logger.info(f"Prepared {len(update_payloads)} records for resync")
            print(f"  Actualizando {len(update_payloads)} órdenes...")
            ...
            for i in range(0, len(update_payloads), BATCH_SIZE):
                ...
                batch_num = i // BATCH_SIZE + 1
                print(f"  Batch {batch_num} ({len(batch)} registros)...", end=" ", flush=True)

                for attempt in range(MAX_RETRIES):
                    try:
                        response = self.session.patch(
                            API_BASE_URL + "/api/production_orders/batch_update",
                            json={"production_orders": batch},
                            headers={
                                'Content-Type': 'application/json',
                                'X-Company-Token': COMPANY_TOKEN
                            },
                        ...
                        if response.status_code == 200:
                            data = response.json()
                            batch_updated = data.get('updated_count', 0)
                            batch_results = data.get('results', [])
                            batch_not_found = sum(1 for r in batch_results if r.get('status') == 'not_found')
                            ...
                            print(f"OK ({batch_updated} actualizados, {batch_not_found} no encontrados)")
                            break
                        else:
                            logger.warning(f"Batch {batch_num} error {response.status_code}: {response.text[:200]}")
                            if attempt < MAX_RETRIES - 1:
                                time.sleep(2 ** attempt)
                    except Exception as e:
            ...
            logger.info(f"Resync completado: {updated_total} actualizados, {not_found_total} no encontrados, {error_total} errores")
            print(f"\n  Resync completado:")
            ...
            logger.error(f"Error en resync_product_keys: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return False
...
def main():
    ...
    Flags:
      --resync   Re-procesa todas las órdenes y actualiza product_key en el WMS.
    """
    import argparse
    parser = argparse.ArgumentParser(description="Rzavala DBF Uploader")
    parser.add_argument("--resync", action="store_true",
                        help="Re-sincroniza product_key de todas las órdenes ya subidas")
    args, _ = parser.parse_known_args()

    print("=" * 70)
    print("RZAVALA DBF UPLOADER")
    print("=" * 70)
    ...
    # ── Modo resync ──────────────────────────────────────────────────────────
    if args.resync:
        print("\n>>> MODO RESYNC: actualizando product_key en órdenes existentes...\n")
        logger.info("Running in RESYNC mode")
        success = uploader.resync_product_keys()
        print("\n" + ("✅ Resync completado." if success else "❌ Resync completado con errores. Revisa el log."))
        input("\nPresiona Enter para salir...")
        sys.exit(0 if success else 1)

    # ── Modo persistente normal ──────────────────────────────────────────────
    print(f"Check interval: {CHECK_INTERVAL_SECONDS} seconds")
    print("(Ejecuta con --resync para actualizar claves de producto en órdenes existentes)")
    print()
